...
public:
    ...
    void updateData(std::map<int,sl::Transform>& camera_poses,
    ...
    void disableVisRaw(){show_raw = false;}
    void exit();
    bool stop = false;
    bool fused = false;
...
private:
    ...
    // void createIDRendering(sl::BodyData &bodyData, sl::float4 clr, unsigned int id);
    ...
    std::map<int,PointCloud> pointClouds_;
    std::map<int,sl::Transform> cam_poses;
